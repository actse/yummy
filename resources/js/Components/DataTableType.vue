<template>
    <div class="bg-white p-3 max-w-xl mx-auto m-10 rounded-lg opacity-[90%]">
        <div class="flex justify-between items-center px-4">
            ...
            <div>
                <button @click="idAddTypeProductModalOpen = true" class="flex items-center justify-center w-1/2 px-3 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
      ...
      <table class=" w-full ">
        ...
        <tbody class="divide-y text-xs font-sans text-gray-700 border-b-2 sm:text-sm md:text-base md:font-medium">
          <tr class="h-9 mx-auto hover:bg-gray-50 sm:h-20 md:h-20 lg:h-20" v-for="(item, index) in displayedAndFilteredData " :key="index">
            ...
            <td class="text-left whitespace-pre-line break-words break-all">{{ item.type_product_name }}</td>
            <td class=" px-4 py-4 text-sm ">
                <div class="text-center space-x-6">
                    <button @click="deleteTypeProduct(item.id)" class="text-gray-600 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-400 hover:text-red-500 focus:outline-none">
  ...
  export default {
    ...
    displayedAndFilteredData() {
        const filteredType_product = this.type_product.filter((type_product) => {
