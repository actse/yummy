<template>
    ...
    <div class="bg-white p-3 max-w-xl mx-auto m-10 rounded-lg opacity-[90%]">
        <div class="flex justify-between items-center px-4">
            ...
            <div class="flex space-x-3">
                <button
                    addPackageSecondary
                    @click="idAddPackageSecondaryModalOpen = true"
                    class="flex items-center justify-center w-1/2 px-3 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
        ...
        <table class="w-full">
            ...
            >
                <tr
                    class="h-9 mx-auto hover:bg-gray-50 sm:h-20 md:h-20 lg:h-20"
                    v-for="(item, index) in displayedAndFilteredData_secondary"
                    :key="index"
                >
                    <td class="text-center">{{ index + 1 }}</td>
                    <td
                    ...
                    <td class="px-4 py-4 text-sm">
                        <div class="text-center space-x-6">
                            <button
                                @click="deletePackageSecondary(item.id)"
                                class="text-gray-600 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-400 hover:text-red-500 focus:outline-none"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                            ...
                            <button
                                @click="openEditModalSecondary(item.id)"
                                class="text-gray-600 transition-colors duration-200 dark:hover:text-yellow-500 dark:text-gray-400 hover:text-yellow-500 focus:outline-none"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
    ...
    <MenuModal v-if="isModalEditOpen">
        <div
            class="relative text-gray-800 font-bold border-b-2 border-gray-300"
        >
            <h1>แก้ไขแพ็คเกจเสริม</h1>
            <button>
                <img
                    @click="closeEditModalSecondary"
                    class="absolute w-3 top-0 right-0"
                    src="../../imgs/X.svg"
                />
            </button>
        ...
        <div class="bg-white w-full px-4 border-t-2 pt-3">
            <div class="bg-white w-full px-4 border-t-2 pt-3">
                <form
                    @submit.prevent="updatePackageSecondary"
                    enctype="multipart/form-data"
                    class="w-full max-w-lg"
                >
                    <div
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        <div class="w-full md:w-2/3 px-3 mb-2 md:mb-0">
                            <label
                                ...
                                for="grid-first-name"
                            >
                                กรอกชื่อแพ็คเกจเสริมที่ต้องการเพิ่ม
                            </label>
                            <input
                                class="appearance-none mt-2 block w-full bg-gray-200 text-gray-700 border border-white rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                                id="grid-first-name"
                                type="text"
                                v-model="package_secondary_name"
                            />
                        </div>
                        <div class="w-full md:w-2/3 px-3 mb-2 md:mb-0">
                            <label
                                ...
                                for="grid-first-name"
                            >
                                กรอก ราคา แพ็คเกจเสริมที่ต้องการเพิ่ม
                            </label>
                            <input
                                class="appearance-none mt-2 block w-full bg-gray-200 text-gray-700 border border-white rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                                id="grid-first-name"
                                type="number"
                                v-model="package_secondary_price"
                            />
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-2">
    ...
    <AddPackageSecondaryModalOpen v-if="idAddPackageSecondaryModalOpen">
        <div
            class="relative text-gray-800 font-bold border-b-2 border-gray-300"
        >
            <h1>เพิ่มแพ็คเกจเสริม</h1>
            <button>
                <img
                    @click="closeAddSecondaryPackageModal"
                    class="absolute w-3 top-0 right-0"
                    src="../../imgs/X.svg"
                />
            </button>
        ...
        <div class="bg-white w-full px-4 border-t-2 pt-3">
            <form
                @submit.prevent="addPackageSecondary"
                enctype="multipart/form-data"
                class="w-full max-w-lg"
            >
                <div
...
import AddPackageSecondaryModalOpen from "@/components/MenuModal.vue";
...
export default {
    ...
    data() {
        return {
            ...
            idAddPackageSecondaryModalOpen: false,
            selectedPackage: null,
            package_id: "",
            package_name: "",
            package_price: "",
            package_secondary_id: "",
            package_secondary_name: "",
            package_secondary_price: "",
            products: [],
            type_product: [],
            main_package: [],
            secondary_package: [],
    ...
    computed: {
        ...
        displayedAndFilteredData_secondary() {
            const filtere_Package_secondary = this.secondary_package.filter(
                (secondary_package) => {
                    const nameMatch = secondary_package.title_package
                        .toLowerCase()
                        .includes(this.searchName.toLowerCase());
                    return nameMatch;
                }
            ...
            return filtere_Package_secondary.slice(startIndex, endIndex);
        },
    },
    methods: {
        updatePage(page) {
        ...
        openEditModalSecondary(package_secondary_id) {
            this.isModalEditOpen = true;
            this.selectedPackageSecondary = this.secondary_package.find(
                (secondary_package) =>
                    secondary_package.id === package_secondary_id
            );
            this.package_secondary_id = package_secondary_id;
            this.package_secondary_name =
            this.selectedPackageSecondary.title_package;
            // this.package_price = this.selectedPackageSecondary.package_price;

            this.fetch_package_secondary();
            console.log(package_secondary_id);
        },
        closeEditModal() {
        ...
        closeEditModalSecondary() {
            this.isModalEditOpen = false;
            this.selectedPackage = "";
        },
        closeAddSecondaryPackageModal() {
            this.idAddPackageSecondaryModalOpen = false;
        },
        closeManegModal() {
        ...
        updatePackageSecondary() {
            const formData = new FormData();
            formData.append("package_secondary_id", this.package_secondary_id);
            formData.append(
                "package_secondary_name",
                this.package_secondary_name
            );
            ...
            axios
                .post("/edit_secondary_package", formData)
                .then((response) => {
                    console.log(response);
                    this.fetch_package_secondary();
                    this.isModalEditOpen = false;
        ...
        deletePackageSecondary(id) {
            const formData = new FormData();
            formData.append("package_secondary_id", id);

            axios
                .post("/delete_secondary_package", formData)
                .then((response) => {
                    console.log(response);
                    this.fetch_package_secondary();
                })
        ...
        addPackageSecondary() {
            ...
            formData.append(
                "package_secondary_price",
                this.package_secondary_price
            );

            console.log(this.package_secondary_name);
            console.log(this.package_secondary_price);

            // return;

            axios
                .post("/insert_secondary_package", formData)
                .then((response) => {
                    if ((response.data = true)) {
                        console.log(response);
                        (this.idAddPackageSecondaryModalOpen = false),
